...
This thesis focuses on improving practical reliability for multi-step remote sensing workflows in RS-agent. Based on iterative use during the summer project, three recurring issues were prioritized: ambiguous submission control, manual synchronization overhead, and prompt/context inflation in long interactions.

The contribution of this work is therefore implementation-oriented rather than architecture-replacement-oriented:
\begin{enumerate}
	\item \textbf{HITL confirmation hardening}: strict confirm/cancel gating for submission-sensitive operations, with billing tied only to successful submission.
	\item \textbf{Git synchronization workflow}: repository-state-aware sync logic with explicit handling for fast-forward, divergence, and conflict paths.
	\item \textbf{Tiered skill loading}: layer-1 catalog plus selective layer-2 document injection to reduce unnecessary prompt payload.
\end{enumerate}

These contributions are evaluated in Chapter~\ref{ch:testing-report} with a focus on behavior correctness and operational consistency.
